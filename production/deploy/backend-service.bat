...
echo   backend-service.bat set-public-dir pb_public
...
echo   - set-public-dir configura la carpeta publica de PocketBase ^(ej: pb_public^) para servir el Frontend.
echo   - sync-frontend actualiza frontend\client\config\hub-runtime.json y frontend\client\public\assets\libs\js\env.js.
echo   - prepare-nginx deja una carpeta estatica lista para Nginx por si se usa como proxy secundario.
echo   - PUBLIC_DIR por defecto usa 'pb_public' para que PocketBase sirva el Frontend unificado.
...
if not defined NEXT_PUBLIC_DIR set /p "NEXT_PUBLIC_DIR=Escribe PUBLIC_DIR (ej: pb_public) o off para desactivar [pb_public]: "
if not defined NEXT_PUBLIC_DIR set "NEXT_PUBLIC_DIR=pb_public"
...
if defined PUBLIC_DIR (
  echo [OK] PUBLIC_DIR actualizado a %PUBLIC_DIR%. PocketBase servira el frontend unificado.
) else (
  echo [OK] PUBLIC_DIR desactivado.
)
