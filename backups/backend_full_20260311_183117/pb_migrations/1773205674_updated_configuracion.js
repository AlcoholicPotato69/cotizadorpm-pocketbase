...
migrate((app) => {
  const collection = app.findCollectionByNameOrId("pbc_2165897088")
  ...
  unmarshal({
    ...
    "listRule": "(@request.auth.id != \"\" (   @request.auth.role = \"admin\"   || @request.auth.allowed_tenants ?= tenant   || @request.auth.tenant_default = tenant   || (@request.auth.role = \"plaza_mayor\" && tenant = \"plaza_mayor\")   || (@request.auth.role = \"casa_de_piedra\" && tenant = \"casa_de_piedra\") )) || (@request.auth.id = \"\" && tenant = \"casa_de_piedra\" && (clave = \"premontaje_pct\" || clave = \"hora_extra_cfg\"))",
