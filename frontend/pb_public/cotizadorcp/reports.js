...
function normalizeReportRole(value) {
    ...
    if (role === 'plazamayor' || role === 'plaza_mayor' || role === 'pm' || role === 'finanzas') return 'plaza_mayor';
    if (role === 'casadepiedra' || role === 'casa_de_piedra' || role === 'cp') return 'casa_de_piedra';
    return role;
}

function resolveReportPermissions(profile, authCtx, tenantSlug) {
    ...
    if (ctxPerms) return { role, perms: { ...ctxPerms } };
    ...
    if (role === 'admin') return { role, perms: { access: true, orders_view: true, reports_view: true, clients_view: true, clients_manage: true } };
    if (role === 'verificador') return { role, perms: { access: true, orders_view: false, reports_view: false, clients_view: true, clients_manage: false } };
    ...
    return { role, perms: { access: !!rawPerms.access, ...rawPerms } };
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const access = resolveReportPermissions(profile || {}, authCtx || {}, 'casa_de_piedra');
    const __role = access.role;
