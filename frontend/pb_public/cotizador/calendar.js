...
function pmCalendarNormalizeRole(value) {
    ...
    if (role === 'plazamayor' || role === 'pm' || role === 'finanzas') return 'plaza_mayor';
    if (role === 'casadepiedra' || role === 'cp') return 'casa_de_piedra';
    if (role === 'administrador' || role === 'superadmin' || role === 'super_admin') return 'admin';
    return role;
}

function pmCalendarResolvePermissions(profile, authCtx, tenantSlug) {
    const role = pmCalendarNormalizeRole(
        authCtx?.role
        || profile?.role
        || profile?.rol
        || authCtx?.profile?.role
        || authCtx?.session?.user?.role
        || ''
    );
    const ctxPerms = (authCtx?.permissions && typeof authCtx.permissions === 'object') ? authCtx.permissions : null;
    if (ctxPerms) return { ...ctxPerms };

    const rawPerms = (profile?.app_metadata?.finanzas?.permissions && typeof profile.app_metadata.finanzas.permissions === 'object')
        ? profile.app_metadata.finanzas.permissions
        : {};
    const allowedTenants = Array.isArray(profile?.allowed_tenants)
        ? profile.allowed_tenants.map((tenant) => String(tenant || '').trim().toLowerCase())
        : [];
    const defaultTenant = String(profile?.tenant_default || profile?.default_tenant || '').trim().toLowerCase();
    const tenantAllowed = !tenantSlug || allowedTenants.includes(tenantSlug) || defaultTenant === tenantSlug;

    if (role === 'admin') {
        return { access: true, orders_edit: true, orders_view: true, reports_view: true, clients_view: true, clients_manage: true };
    }
    if (role === 'verificador') {
        return { access: true, orders_edit: false, orders_view: false, reports_view: false, clients_view: true, clients_manage: false };
    }
    if (role === tenantSlug || tenantAllowed) {
        return {
            access: true,
            orders_edit: true,
            orders_view: rawPerms.orders_view !== false,
            reports_view: rawPerms.reports_view !== false,
            clients_view: rawPerms.clients_view !== false,
            clients_manage: rawPerms.clients_manage !== false
        };
    }
    return { access: !!rawPerms.access, ...rawPerms };
}
...
function pmCalendarShowHoverCard(info) {
    ...
    `;
    const cx = Number(info?.jsEvent?.pageX || 0);
    const cy = Number(info?.jsEvent?.pageY || 0);
    const cardW = 260;
    const cardH = 120;
    const docW = document.documentElement.scrollWidth;
    const docH = document.documentElement.scrollHeight;
    const left = (cx + 14 + cardW > docW) ? Math.max(12, cx - cardW - 14) : Math.max(12, cx + 14);
    const top = (cy + 14 + cardH > docH) ? Math.max(12, cy - cardH - 14) : Math.max(12, cy + 14);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const __role = pmCalendarNormalizeRole(authCtx?.role || profile?.role || '');
    myPermissions = pmCalendarResolvePermissions(profile || {}, authCtx || {}, 'plaza_mayor');
