const fs = require('fs');
const fp = 'frontend/client/system/config.html';
let text = fs.readFileSync(fp, 'utf8');

const navbarSearch = `        <button type="button" id="menu-materials" onclick="switchView('v_materials'); return false;" class="tool-tab hidden">
          <i class="fa-solid fa-palette"></i> Config de espacio
        </button>`;

const navbarReplace = navbarSearch + `
        <button type="button" id="menu-users" onclick="switchView('v_users'); return false;" class="tool-tab">
          <i class="fa-solid fa-users"></i> Usuarios
        </button>`;

if (text.includes(navbarSearch)) {
    text = text.replace(navbarSearch, navbarReplace);
    console.log('Navbar patched');
} else {
    console.log('Navbar search failed');
}

const mainEndSearch = '  </main>';
...
if (text.includes(mainEndSearch)) {
    text = text.replace(mainEndSearch, vUsersHtml + '\n' + modalHtml);
    console.log('Main patched');
} else {
    console.log('Main search failed');
}

const switchSearch = "if(viewId === 'v_docs') await loadDocsConfig();";
const switchReplace = switchSearch + "\n      if(viewId === 'v_users') await loadUsers();";

if (text.includes(switchSearch)) {
    text = text.replace(switchSearch, switchReplace);
    console.log('Switch patched');
} else {
    console.log('Switch search failed');
}
...
const funcInsertSearch = "      // default view\n      await switchView('v_concepts');";

if (text.includes(funcInsertSearch)) {
    text = text.replace(funcInsertSearch, jsFuncs + '\n' + funcInsertSearch);
    console.log('Func patched');
} else {
    console.log('Func search failed');
}
