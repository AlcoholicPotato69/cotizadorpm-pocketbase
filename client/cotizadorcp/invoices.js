...
window.validateAndSaveInvoice = async function() {
    ...
    try {
        ...
        // 1. Subir Archivos
        const { error: err1 } = await window.globalPocketBase.storage.from('documentos-cp').upload(xmlPath, files.xml);
        if (err1) throw err1;
        const { error: err2 } = await window.globalPocketBase.storage.from('documentos-cp').upload(pdfPath, files.pdf);
        if (err2) throw err2;
...
window.downloadFile = async function(type) {
    ...
    const { data } = await window.globalPocketBase.storage.from('documentos-cp').createSignedUrl(path, 60);
...
window.deleteInvoice = function() {
    window.openConfirm("¿Eliminar factura? Esto desbloqueará la edición.", async () => {
        ...
        try {
            ...
            await window.globalPocketBase.storage.from('documentos-cp')
