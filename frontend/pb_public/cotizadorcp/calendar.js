...
function showCalendarHoverCard(info) {
    ...
    const cardW = 270;
    const cardH = 120;
    const left = (pageX + 14 + cardW > docW) ? Math.max(12, pageX - cardW - 14) : Math.max(12, pageX + 14);
    const top = (pageY + 14 + cardH > docH) ? Math.max(12, pageY - cardH - 14) : Math.max(12, pageY + 14);
...
function resolvePermissions(profile) {
    return resolvePermissionsFromContext(profile, null);
}

function normalizeCalendarRole(value) {
...
function resolvePermissionsFromContext(profile, authCtx) {
    const role = normalizeCalendarRole(
        authCtx?.role
        || profile?.role
        || profile?.rol
    ...
    const tenantAllowed = allowedTenants.includes(CP_CALENDAR_CONFIG_TENANT) || defaultTenant === CP_CALENDAR_CONFIG_TENANT;

    if (role === 'admin') return { access: true, orders_edit: true, orders_view: true, reports_view: true, clients_view: true, clients_manage: true };
    if (role === 'verificador') return { access: true, orders_edit: false, orders_view: false, reports_view: false, clients_view: true, clients_manage: false };
    if (role === CP_CALENDAR_CONFIG_TENANT || tenantAllowed) {
        return {
            access: true,
            orders_edit: rawPerms.orders_edit !== false,
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    myPermissions = resolvePermissionsFromContext(profileRes.data || {}, authCtx || {});
