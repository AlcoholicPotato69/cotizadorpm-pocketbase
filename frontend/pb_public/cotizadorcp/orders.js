...
function __orderNormalizeAccessRole(value) {
...
window.loadOrders = async function () {
    ...
    if (error) {
        ...
        allOrders = (data || [])
            .filter((row) => {
                const rt = __orderNormalizeTenantSlug(row?.tenant);
                return !rt || rt === tenant;
            })
            .map(__orderHydrateQuotePricing);
    }
...
window.processSaveOrder = async function (options = {}) {
    ...
    if (!__orderCanEditOrders()) {
        if (!silent) window.showToast(__orderReadOnlyMessage(), "error");
        return false;
    }
    ...
    try {
        ...
        if (!relaxed) {
            ...
            const conflict = __orderQuoteSpaces.find(cfg => {
                ...
                if (__orderIsPublicidadSpace(cfg)) {
                    const space = __orderGetSpaceById(cfg.spaceId);
                    const digitalCfg = __orderResolveDigitalMediaConfig(cfg, space);
                    const sType = String(space?.tipo_medio || space?.media_type || space?.tipo || '').toLowerCase();
                    if (digitalCfg.enabled || sType === 'digital' || sType.includes('digital') || sType.includes('pantalla') || sType.includes('video')) return false;
                }
...
function __orderScheduleAutoSave() {
    ...
    if (!__orderCanEditOrders()) return;
...
window.previewOrderForGeneration = async function (id) {
    if (!__orderCanEditOrders()) return window.showToast(__orderReadOnlyMessage(), "error");
...
window.openDocsModal = function (id) {
    ...
    } else if (['aprobada', 'finalizada'].includes(order.status) && __orderCanEditOrders()) {
...
window.getOrderHTML = function (o, type) {
    ...
    if (detailSpaces.length > 0) {
        detailSpaces.forEach((sp) => {
            ...
            if (spObj && fi && ff && !detailIsPublicidad && !isConvenio) {
                ...
                breakdown.details.forEach((day, idx) => {
                    rentalRows += `<tr><td class="py-2 px-3 align-top text-[11px] text-gray-700 leading-snug break-words"><span class="font-bold">${__orderEscapeHtml(identity.nombre)}</span> - Renta ${day.dayName}${(idx === 0 && identity.clave) ? `<br><span class="text-[9px] text-gray-400 italic">${__orderEscapeHtml(identity.clave)}</span>` : ''}</td>${orderIsPublicidad ? '<td class="py-2 px-3 text-center text-[10px] text-gray-400">--</td><td class="py-2 px-3 text-center text-[10px] text-gray-400">--</td>' : ''}<td class="py-2 px-3 text-center text-[10px] text-gray-500">${__orderBuildDateCellHtml(day.date, '', sid)}</td><td class="py-2 px-3 text-right font-bold text-[11px] text-gray-700">${__orderFormatMoneyHtml(day.price)}</td></tr>`;
                });
            } else {
        ...
        if (__orderIsPublicidadSpace(space) || isConvenio) {
            ...
            convenioBaseTotal = rentalTotal;
            dayBreakdown.details.forEach((day, idx) => { rentalRows += `<tr><td class="py-2 px-3 align-top text-[11px] text-gray-700 leading-snug break-words"><span class="font-bold">${__orderEscapeHtml(identity.nombre)}</span> - Renta ${day.dayName}${idx === 0 && identity.clave ? `<br><span class="text-[9px] text-gray-400 italic">${__orderEscapeHtml(identity.clave)}</span>` : ''}</td>${orderIsPublicidad ? '<td class="py-2 px-3 text-center text-[10px] text-gray-400">--</td><td class="py-2 px-3 text-center text-[10px] text-gray-400">--</td>' : ''}<td class="py-2 px-3 text-center text-[10px] text-gray-500">${__orderBuildDateCellHtml(day.date, '', space.id || o.espacio_id)}</td><td class="py-2 px-3 text-right font-bold text-[11px] text-gray-700">${__orderFormatMoneyHtml(day.price)}</td></tr>`; });
        }
    } else {
    ...
    const tableHeadHtml = isConvenio
        ? (orderIsPublicidad
            ? `<thead class="cp-pdf-table-head bg-gray-100 text-sm font-black text-gray-500 uppercase"><tr><th class="py-2 px-3 text-left rounded-l">Concepto</th><th class="py-2 px-3 text-center">${__cpSafeHtml(detailColumnLabel)}</th><th class="py-2 px-3 text-center">Medidas / Duración</th><th class="py-2 px-3 text-center rounded-r">Fecha</th></tr></thead>`
            : `<thead class="cp-pdf-table-head bg-gray-100 text-sm font-black text-gray-500 uppercase"><tr><th class="py-2 px-3 text-left rounded-l">Concepto</th><th class="py-2 px-3 text-center rounded-r">Fecha</th></tr></thead>`)
        : (orderIsPublicidad
            ? `<thead class="cp-pdf-table-head bg-gray-100 text-sm font-black text-gray-500 uppercase"><tr><th class="py-2 px-3 text-left rounded-l">Concepto</th><th class="py-2 px-3 text-center">${__cpSafeHtml(detailColumnLabel)}</th><th class="py-2 px-3 text-center">Medidas / Duración</th><th class="py-2 px-3 text-center">Fecha</th><th class="py-2 px-3 text-right rounded-r">Importe</th></tr></thead>`
            : `<thead class="cp-pdf-table-head bg-gray-100 text-sm font-black text-gray-500 uppercase"><tr><th class="py-2 px-3 text-left rounded-l">Concepto</th><th class="py-2 px-3 text-center">Fecha</th><th class="py-2 px-3 text-right rounded-r">Importe</th></tr></thead>`);
...
const __ORDER_MAX_DISCOUNT_PERCENT = 10;
...
function __orderIsBlockedDate(space, dateStr, guests) {
    if (space) {
        try {
            const b2b = typeof space.config_b2b === 'string' ? JSON.parse(space.config_b2b) : space.config_b2b || {};
            const dm = b2b.digital_media || b2b.digitalMedia || b2b.medio_digital || {};
            const isDigitalMedia = !!(dm.enabled ?? dm.activo ?? dm.es_digital);
...
function __orderSyncActiveSpaceModeUi(cfg) {
    ...
    const horarioField = document.getElementById('oed-horario')?.parentElement;
    const horarioCustom = document.getElementById('oed-horario-custom');
    const horarioValue = document.getElementById('oed-horario')?.value || '';
    ...
    if (horarioField) horarioField.classList.toggle('hidden', isPublicidad);
    if (horarioCustom) horarioCustom.classList.toggle('hidden', isPublicidad || horarioValue !== 'personalizado');
    if (isPublicidad) {
...
function __orderNormalizeGlobalAdjustment(subtotalRaw = 0, options = {}) {
    const opts = options && typeof options === 'object' ? options : {};
    ...
    const isPercent = String(unitEl?.value || 'fixed') === 'percent';
    const subtotal = Math.max(0, parseFloat(subtotalRaw || 0) || 0);
    const maxFixedDiscount = parseFloat((subtotal * (__ORDER_MAX_DISCOUNT_PERCENT / 100)).toFixed(2));
    let adjVal = Math.max(0, parseFloat(valueEl?.value || 0) || 0);
    let message = '';

    if (adjType === 'descuento') {
        if (isPercent && adjVal > __ORDER_MAX_DISCOUNT_PERCENT) {
            adjVal = __ORDER_MAX_DISCOUNT_PERCENT;
            message = `El descuento maximo permitido es ${__ORDER_MAX_DISCOUNT_PERCENT}%.`;
        } else if (!isPercent && subtotal <= 0 && adjVal > 0) {
            adjVal = 0;
            message = 'No se puede aplicar descuento sobre un subtotal en 0.';
        } else if (!isPercent && adjVal > maxFixedDiscount) {
            adjVal = maxFixedDiscount;
            message = `El descuento maximo permitido para este subtotal es ${__orderFormatMoneyMx(maxFixedDiscount)}.`;
        }
    }
    ...
    return {
        ...
        isPercent,
        maxFixedDiscount,
        adjustment: adjType === 'ninguno' ? 0 : (isPercent ? subtotal * (adjVal / 100) : adjVal)
    };
}
...
function __orderDescribePublicityPricingRule(cfgLike = {}, space = null, baseOverride = undefined) {
    ...
    if (cfgLike?.convenioEnabled) {
        const isCustom = cfgLike?.customPermanence || (cfgLike?.endDate && days !== 30);
        const bloqueoValue = isCustom ? `Personalizado (${days} días)` : 'Automático (30 días)';
        return {
            modeLabel: 'Convenio',
            detailRows: [
                { label: 'Bloqueo del espacio', value: bloqueoValue },
...
function __orderDescribePublicityPricingRule(cfgLike = {}, space = null, baseOverride = undefined) {
    ...
    if (cfgLike?.convenioEnabled) {
        ...
        return {
            ...
            explanation: `El espacio queda bloqueado al precio base del catalogo: ${__orderFormatMoneyMx(resultBase)}.`
        };
    }
    if (cfgLike?.customPriceEnabled) {
        if (String(cfgLike?.customPriceMode || 'total') === 'per_day') {
            return {
                modeLabel: 'Precio personalizado por dia',
                detailRows: [
                    { label: 'Precio manual por dia', value: __orderFormatMoneyMx(manualValue) },
                    { label: 'Dias seleccionados', value: `${days} dia(s)` }
                ],
                explanation: `${__orderFormatMoneyMx(manualValue)} x ${days} dia(s) = ${__orderFormatMoneyMx(resultBase)}`
            };
        }
        return {
            modeLabel: 'Precio personalizado total',
    ...
    if (cfgLike?.customPermanence) {
        return {
            modeLabel: 'Estancia personalizada por dia',
            detailRows: [
                { label: 'Precio catalogo por dia', value: __orderFormatMoneyMx(catalogBase) },
                { label: 'Dias seleccionados', value: `${days} dia(s)` },
                { label: 'Resultado calculado', value: __orderFormatMoneyMx(resultBase) }
            ],
            explanation: `${__orderFormatMoneyMx(catalogBase)} x ${days} dia(s) = ${__orderFormatMoneyMx(resultBase)}`
        };
    }
    return {
        modeLabel: 'Periodo automatico (30 dias)',
        detailRows: [{ label: 'Dias aplicados', value: '30 dia(s)' }],
        explanation: `Periodo automatico de 30 dia(s) = ${__orderFormatMoneyMx(resultBase)}`
    };
}
...
function __orderUpdateSpaceConfigTitle() {
    ...
    const copy = document.getElementById('oed-space-config-copy');
    const baseLabel = __orderFormatMoneyMx(__orderResolveDisplayedBase(cfg, space));
    if (title) title.textContent = `Configuracion del Espacio - ${space?.nombre || cfg?.spaceId || '--'} - ${baseLabel}`;
    if (copy) copy.textContent = isPublicidad
        ? 'Controla nombre de la cotizacion, fechas, material, medidas y configuracion comercial del espacio activo.'
        : 'Controla nombre de la cotizacion, fechas, asistentes, horario y complementos del espacio activo.';
}
...
async function __orderRenderDatePicker() {
    ...
    const isDigital = (() => {
        ...
        const digitalCfg = __orderResolveDigitalMediaConfig(cfg, space);
        const sType = String(space?.tipo_medio || space?.media_type || space?.tipo || '').toLowerCase();
        return digitalCfg.enabled || sType === 'digital' || sType.includes('digital') || sType.includes('pantalla') || sType.includes('video');
    })();
    const reserved = isDigital ? new Set() : __orderGetReservedDatesForSpace(cfg.spaceId);
...
window.abrirModalMontaje = async function (prefix) {
    ...
    state.reserved = isDigital ? new Set() : __orderGetReservedDatesForSpace(cfg.spaceId);
...
window.updateSummaryUI = function () {
    ...
    const hideOperationalSummary = spaces.length > 0 && nonPublicidadSpaces.length === 0;
    ['sum-guests', 'sum-schedule', 'sum-montaje', 'sum-hextras'].forEach((id) => {
        const cell = document.getElementById(id);
        const row = cell?.closest('.flex') || cell?.parentElement;
        if (row) row.classList.toggle('hidden', hideOperationalSummary);
    });
...
window.recalcTotal = function () {
    ...
    __orderQuoteSpaces.forEach(cfg => {
        ...
        const isDigital = (() => {
            if (!isPublicidad) return false;
            const digitalCfg = __orderResolveDigitalMediaConfig(cfg, space);
            const sType = String(space?.tipo_medio || space?.media_type || space?.tipo || '').toLowerCase();
            return digitalCfg.enabled || sType === 'digital' || sType.includes('digital') || sType.includes('pantalla') || sType.includes('video');
        })();
        const blockedOk = isDigital ? true : !__orderCfgHasBlockedDates(cfg);
    ...
    const renderPublicityPricingCard = (sp, isActive) => {
        ...
        return `<button type="button" onclick="window.selectOrderQuoteSpace('${sp.spaceId}')" class="w-full text-left rounded-lg border ${summaryStateClass(isActive)} p-3 space-y-3 transition hover:border-brand-red">
            ...
            <div class="rounded-xl border border-gray-200 bg-white/80 p-3 space-y-1.5">
                ...
                <div class="flex justify-between text-[10px] text-gray-500"><span>Precio base catalogo</span><span>${__orderFormatMoneyMx(__orderGetSpaceById(sp.spaceId)?.precio_base || 0)}</span></div>
                ${detailRows}
    ...
    const normalizedAdj = convenioMode
        ? { adjType: 'ninguno', adjVal: 0, isPercent: false, adjustment: 0 }
        : __orderNormalizeGlobalAdjustment(subtotalRaw, { showToast: true });
    const adjType = normalizedAdj.adjType;
    const adjVal = normalizedAdj.adjVal;
    const isPercent = normalizedAdj.isPercent;
...
window.getFormDataFromModal = function () {
    ...
    const normalizedAdj = hasConvenioOrder
        ? { adjType: 'ninguno', adjVal: 0, isPercent: false }
        : __orderNormalizeGlobalAdjustment(__orderTotals?.subtotal || 0, { showToast: true });
    const adjType = normalizedAdj.adjType;
    const adjVal = normalizedAdj.adjVal;
    const isPercent = normalizedAdj.isPercent;
...
window.openOrderEditModal = async function (id) {
    ...
    const isReadOnly = !__orderCanEditOrders();
    const isLocked = ['aprobada', 'finalizada'].includes(order.status);
    const editorLocked = isLocked || isReadOnly;
    document.querySelectorAll('#order-edit-modal input, #order-edit-modal select').forEach(i => { if (i.id !== 'btn-save-progress') i.disabled = editorLocked; });
    ['oed-quote-name', 'oed-status'].forEach(id => { const el = document.getElementById(id); if (el) el.disabled = editorLocked; });
    document.getElementById('oed-btn-montaje').disabled = editorLocked;
