...
window.loadOrders = async function() {
    ...
    if (error) {
        ...
        allOrders = (data || [])
            .filter((row) => {
                const rt = __pmNormalizeTenantSlug(row?.tenant);
                return !rt || rt === tenant;
            })
            .map(__pmHydrateOrderPricing);
    }
...
(function () {
  ...
  const __pmIsDigitalSpace = (space) => {
    if (!space) return false;
    const sType = String(space.tipo_medio || space.media_type || space.tipo || '').toLowerCase();
    if (sType === 'digital' || sType.includes('digital') || sType.includes('pantalla') || sType.includes('video')) return true;
    const norm = (v) => !!(v?.enabled ?? v?.activo ?? v?.es_digital);
    let b2b = {};
    try { b2b = typeof space.config_b2b === 'string' ? JSON.parse(space.config_b2b) : (space.config_b2b || {}); } catch(e){}
    if (norm(b2b.digital_media || b2b.digitalMedia || b2b.medio_digital)) return true;
    return false;
  };
  ...
  async function renderOrderDatePicker() {
    ...
    const cfg = activeCfg();
    if (!cfg) return;
    const spaceObj = getSpace(cfg.spaceId);
    const reserved = (__pmIsPublicidadSpace(spaceObj) && __pmIsDigitalSpace(spaceObj)) ? new Set() : getReservedDatesForSpace(cfg.spaceId);
  ...
  async function findConflict(orderId) {
    ...
    for (const cfg of selected) {
      const spaceObj = getSpace(cfg.spaceId);
      if (__pmIsPublicidadSpace(spaceObj) && __pmIsDigitalSpace(spaceObj)) continue;
